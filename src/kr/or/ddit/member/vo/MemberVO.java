...
public class MemberVO {
	...
	/*
	 mvc -> model view(화면에 보여지는 것) controller(java logic 처리) 분리해서 작어 
	 	    dao(db와 연결)/service(db와 직접 연결하지 않는 일반적인 logic 처리)
	 	    model (VO / service,dao)
	 	    
	 	    interface를 만드는 이유 -> 표준화를 위해서 만든다
	 	    
	 singleton -> 하나의 객체를 공유해서 사용 (static 으로 선언해서 사용)
